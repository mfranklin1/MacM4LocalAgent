...
if [[ -f "$OUT" ]]; then
  ...
  PREV_LITELLM_MASTER_KEY="${LITELLM_MASTER_KEY:-}"
fi
...
# Generate a strong random LITELLM_MASTER_KEY on first install. Preserved
# across re-runs of `make detect` via PREV_LITELLM_MASTER_KEY above so a
# rotation requires deliberately deleting the old line. Never echoes the
# value to logs.
generate_master_key() {
  if command -v openssl >/dev/null 2>&1; then
    printf 'sk-litellm-%s' "$(openssl rand -hex 24)"
  elif command -v python3 >/dev/null 2>&1; then
    python3 -c 'import secrets; print("sk-litellm-" + secrets.token_hex(24))'
  else
    # If we hit this branch the user almost certainly has bigger problems
    # than weak entropy, but use date+pid as a last-resort fallback so the
    # key is at least unique per machine and not the literal placeholder.
    printf 'sk-litellm-fallback-%s-%s' "$(date +%s)" "$$"
  fi
}
LITELLM_MASTER_KEY="${PREV_LITELLM_MASTER_KEY:-$(generate_master_key)}"
