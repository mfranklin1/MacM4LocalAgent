...
| LiteLLM master key          | `$LITELLM_MASTER_KEY` — see `config/detected.env`      |
...
2. Set **Base URL** to:
   ...
   ```
3. Set **API Key** to the value from `config/detected.env`. Fetch it with:

   ```bash
   grep LITELLM_MASTER_KEY config/detected.env | cut -d= -f2 | tr -d '"' | pbcopy
   ```

   then paste. The key is host-specific and randomly generated by
   `make detect`; it is not committed to the repo.
