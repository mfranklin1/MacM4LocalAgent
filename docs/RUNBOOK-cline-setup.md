...
Fix (superseded 2026-07-02 — see below): the Modelfile generated by
`download-ollama-from-hf.sh` originally gained the canonical Qwen3
ChatML template plus `<|im_end|>`/`<|im_start|>` as stop tokens.

**Current fix — RENDERER/PARSER (2026-07-02).** The handwritten ChatML
template fixed the role-marker hallucination but left the model
registered with `Capabilities: completion` only: no tools section is
rendered, Ollama's OpenAI endpoint rejects `tools`, and any tool call
the model attempts reaches the harness as raw JSON *text* that Cline
can't execute (the "tool-call-as-text" stall — cost.db requests
5031/5033/5034). Ollama (>= 0.12; verified on 0.21.2) ships a built-in
`qwen3-coder` renderer + parser that emits the full canonical chat
format *including tools* and parses the model's native tool-call output
into structured OpenAI `tool_calls`. `download-ollama-from-hf.sh` now
writes RENDERER/PARSER instead of TEMPLATE. Existing installs can be
upgraded in place without re-downloading the GGUF:

```bash
GGUF_PATH=$(ollama show qwen3-coder-next:q4 --modelfile \
...
ollama create qwen3-coder-next:q4 -f /tmp/qwen3.modelfile
launchctl kickstart -k "gui/$(id -u)/com.local.litellm"

# Verify: "tools" must appear under Capabilities
ollama show qwen3-coder-next:q4 | sed -n '/Capabilities/,+3p'
```
